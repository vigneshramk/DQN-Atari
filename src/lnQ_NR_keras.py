...
class QNetwork():
    ...
    def __init__(self, environment_name):
        ...
        if(environment_name == 'CartPole-v0'):
            ...
            self.model = Sequential()
            self.model.add(Dense(action_size, activation='linear',input_dim=state_size))
    ...
    def save_model_weights(self, count):
        ...
        if not os.path.exists(os.path.join(self.environment_name, 'lnQ_NR_weights')):
            os.makedirs(os.path.join(self.environment_name, 'lnQ_NR_weights'))
        time_now = self.environment_name + str(count) + '.h5'
        time_now.replace(" ","")

        file_name = os.path.join(self.environment_name, 'lnQ_NR_weights', time_now)
...
class lnQ_NR_Agent():
    ...
    def __init__(self, environment_name, render=False):
        ...
        str_c = './tmp/'+ environment_name + '/lnQ_NR-experiment/' + time_now
        ...
        for i in range(self.num_episodes):
            ...
            for j in range(self.num_iter):
                try:
                    ...
                    newState = np.reshape(newState, [1,newState.size])
                    Qnext = self.mainQN.model.predict(newState)

                    tarQ = deepcopy(Q0)

                    if(done==False):
                        tarQ[0][action] = reward + self.gamma*(np.max(Qnext))
                    else:
                        tarQ[0][action] = reward

                    self.mainQN.model.fit(s, tarQ, epochs=1, verbose=0)
            ...
            # Save the model weights after every 30 episodes
            if(i%30==0):
                if not os.path.exists(os.path.join(self.environment_name, 'lnQ_NR_Plot_train')):
                    os.makedirs(os.path.join(self.environment_name, 'lnQ_NR_Plot_train'))

                file_name = 'Plot_instant_total_reward_' + str(g_time) + '.png'
                ax1.figure.savefig(os.path.join(self.environment_name, 'lnQ_NR_Plot_train', file_name))
                file_name = 'Plot_average_total_reward_' + str(g_time) + '.png'
                ax2.figure.savefig(os.path.join(self.environment_name, 'lnQ_NR_Plot_train', file_name))
    ...
    def test(self, render, num_episodes=20, model_file=None):
        ...
        if not os.path.exists(os.path.join(self.environment_name, 'lnQ_NR_Plot_test')):
            os.makedirs(os.path.join(self.environment_name, 'lnQ_NR_Plot_test'))
        
        file_name = 'Plot_average_total_reward_' + str(g_time) + '.png'
        self.ax3.figure.savefig(os.path.join(self.environment_name, 'lnQ_NR_Plot_test', file_name))
    ...
    def test_final(self, render=1, done=False, model_file=None):
        ...
        if not os.path.exists(os.path.join(self.environment_name, 'lnQ_NR_Plot_test_final')):
            os.makedirs(os.path.join(self.environment_name, 'lnQ_NR_Plot_test_final'))
        
        file_name = 'Plot_reward_over_episodes_' + str(g_time) + '.png'
        self.ax5.figure.savefig(os.path.join(self.environment_name, 'lnQ_NR_Plot_test_final', file_name))

    def burn_in_memory(self,environment_name):
        # Initialize your replay memory with a burn_in number of episodes / transitions.
        self.replay = Replay_Memory(environment_name)

    def save_final_weight(self):
        self.mainQN.save_model_weights('final')


def lnQ_NR_main(args):
    ...
    # You want to create an instance of the lnQ_NR_Agent class here, and then train / test it.
    agentlnQ_NR = lnQ_NR_Agent(environment_name)

    if args.train == 1:
            agentlnQ_NR.train(args.render)
            agentlnQ_NR.test_final(args.render, True)
            agentlnQ_NR.save_final_weight()
            sys.exit(0)

    else:
        if args.model_file is not None:
            agentlnQ_NR.test_final(args.render, True, args.model_file)
